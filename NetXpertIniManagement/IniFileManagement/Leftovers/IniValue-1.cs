...
using NetXLMP.Extensions;
using NetXpertExtensions;

namespace IniFileManagement
{
	public abstract partial class IniLineValueBase
	{
		#region Properties
		...
		/// <summary>Specifies whether the data will be encrypted <i>when <u>written</u></i>.</summary>
		/// <remarks>
		/// If set to <b>true</b>, the data will be decrypted when it's originally instantiated and thereafter it is always
		/// managed internally in an unencrypted form. Encryption is only performed when the value is de-referenced via the 
		...
		/// <remarks>This is *always* stored as an unencrypted value!</remarks>
		...
		#region Constructors
		protected IniLineValueBase( bool encrypted = false, QuoteTypes quoteType = QuoteTypes.None, char[]? quoteChars = null ) 
		{ 
		...
		protected IniLineValueBase( string value, bool encrypted = false, QuoteTypes quoteType = QuoteTypes.None, char[]? customQuotes = null )
		{
			this._encrypted = encrypted;
			this._quoteType = quoteType;
			value = UnwrapString( value, quoteType, customQuotes );
			this._value = encrypted ? AES.DecryptStringToString( value ) : value;
		}
		#endregion

		#region Accessors
		public string Value { get => this._value; protected set => this._value = value; }
		...
		{
			string data = this._value is null ? string.Empty : this._value.ToString();
			data = this._encrypted ? AES.EncryptStringToString( data ) : data;
			return WrapValue( data, this._quoteType, this._customQuotes );
		}
		...
		/// <summary>Parses a supplied string against all known <i>QuoteTypes</i> and endeavours to detect if one applies.</summary>
		...
		public static Type DetectType( string data )
		{
				...
				foreach( var assembly in AppDomain.CurrentDomain.GetAssemblies() )
				{
					...
					if ( types.Length > 0 )
						foreach ( var t in types )
							if ( !t.IsAbstract && !string.IsNullOrEmpty(t.Namespace) && t.Namespace.Equals( "IniFileManagement" ) && t.IsDerivedFrom<IniLineValueBase>() )
							{
								if ( t != typeof( IniLineStringValue ) )
									try
									{
										dynamic v = Activator.CreateInstance( t, data );
										return t;
									}
									catch ( ArgumentException ) { }

								return typeof( IniLineStringValue );
							}
				}
		...
		public static dynamic CreateInstance( string data, bool encrypted = false, QuoteTypes quoteType = QuoteTypes.None, char[]? quoteChars = null )
		{
			Type t = DetectType( data );
			return Activator.CreateInstance( t, data, encrypted, quoteType, quoteChars );
		}
	...
	public abstract partial class IniLineValue<T> : IniLineValueBase
	{
		#region Constructors
