...
namespace NetXpertExtensions
{
	...
	public static class GraphicsXtensions
	{
		...
		public static Bitmap ResizeTo( this Image source, SizeF size, Brush withBrush = null, bool maintainAspect = true )
		{
			size = new SizeF( Math.Abs( size.Width ), Math.Abs( size.Height ) );
			if ( withBrush is null ) withBrush = new SolidBrush( Color.Transparent );

			Bitmap result = new( (int)size.Width, (int)size.Height );
			using ( Graphics g = Graphics.FromImage( result ) )
			{
				...
				if ( maintainAspect )
				{
					...
					scale = new SizeF( source.Width * scaleFactor, source.Height * scaleFactor );
				}
				else
				{
					SizeF scaleFactor = new( size.Width / source.Width, size.Height / source.Height );
					scale = new SizeF( source.Width * scaleFactor.Width, source.Height * scaleFactor.Height );
				}
		...
		public static Bitmap ResizeTo( this Image source, int width, int height, Brush withBrush = null, bool maintainAspect = true ) =>
			source.ResizeTo( new SizeF( width, height ), withBrush, maintainAspect );
		...
		public static Bitmap ResizeTo( this Image source, int width, int height, Color fillColor, bool maintainAspect = true ) =>
			source.ResizeTo( new SizeF( width, height ), new SolidBrush( fillColor ), maintainAspect );
		...
		/// <summary>Facilitates finding out the size of an image when stored as the specified <seealso cref="ImageFormat"/></summary>
		/// <param name="format">The <seealso cref="ImageFormat"/> to encode the image as for the calculation.</param>
		/// <returns>The size, in bytes, of the image, when encoded in the specified <seealso cref="ImageFormat"/>.</returns>
		public static ulong GetSize( this Image source, ImageFormat format )
		{
			ulong result = 0;
			if (source is not null )
				using (MemoryStream stream = new( source.Width * source.Height ) ) 
				{
					source.Save( stream, format );
					result = (ulong)stream.Length;
				}
			return result;
		}
