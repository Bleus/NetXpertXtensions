...
namespace NetXpertCodeLibrary
{
	public sealed class xXmlNode
	{
		...
		public static implicit operator XmlNode(xXmlNode source) => source?.ToXmlNode();

		public static implicit operator string(xXmlNode source ) =>
			source is null ? string.Empty : source.OuterXml;
		...
		{
			get => index.InRange( Count, 0, BoundaryRule.Loop ) ? this._children[ index ] : null;
			private set
			{
				if ( index.InRange( Count, 0, BoundaryRule.Loop) )
				{
					if ( value is null )
