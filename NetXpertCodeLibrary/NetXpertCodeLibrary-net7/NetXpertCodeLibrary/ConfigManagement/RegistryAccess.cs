...
{
	/// <summary>A static class that provides simple routines for reading/writing to the Registry.</summary>
	/// <remarks>
	/// Functions:
	///		private static RegistryKey ConvertToKey(RegistryHive? hive = null, RegistryView? view = null)
	...
	///		public static bool HasSubKeys(string subKey = "", RegistryKey hive = null);
	...
	///		public static RegistryKey FetchKey(string subKey = "", RegistryHive? hive = null, RegistryView? view = null);
	...
	public static class RegMgmt
	{
		...
		/// <summary>Retrieves all subkeys found in a specfied Registry location in an array.</summary>
		...
		/// <param name="hive">A RegistryKey value specifying which hive to use. (Defaults to the value in RegistryAccess.DEFAULT_REGISTRY_KEY)</param>
		/// <returns>
		...
		/// <summary>Favilitates setting the ACL permissions on a specified RegistryKey.</summary>
		/// <param name="subkey">A string specifying the subkey to modify.</param>
		/// <param name="access">A RegistrySecurity object specifying the permission to set.</param>
		/// <param name="hive">A RegistryHive value specifying which Hive to use. (Defaults to DEFAULT_REGISTRY_HIVE)</param>
		/// <param name="view">A RegistryView value specifying which Registry view to reference. (Defaults to DEFAULT_REGISTRY_VIEW)</param>
		...
		/// <param name="subKey">A string specifying the SubKey to dereference. (Defaults to DEFAULT_SUBKEY)</param>
		/// <param name="hive">A RegistryKey value specifying which hive to use. (Defaults to DEFAULT_REGISTRY_HIVE)</param>
		/// <returns>
		...
		/// <summary>Returns the number of subkeys found in the specified RegistryKey + subkey</summary>
		/// <param name="subKey">A string specifying the SubKey to dereference. (Defaults to the value in RegistryAccess.DEFAULT_SUBKEY)</param>
		/// <param name="hive">A RegistryKey value specifying which hive to use. (Defaults to HKEY_LOCAL_MACHINE)</param>
		/// <param name="view">A RegistryView value specifying which Registry view to reference.</param>
		/// <returns>A count of the number of keys/subkeys found in the requested registry location. Or -1 if the request failed for any reason.</returns>
		/// <remarks>Any errors encountered in the process are stored in RegistryKey.LastError.</remarks>
		public static int SubKeyCount(string subKey = "", RegistryHive? hive = null, RegistryView? view = null) =>
		...
		/// <summary>Returns the number of value entries found in the specified RegistryKey + subkey.</summary>
		/// <param name="subKey">A string specifying the SubKey to dereference. (Defaults to the value in RegistryAccess.DEFAULT_SUBKEY)</param>
		/// <param name="hive">A RegistryHive value specifying which Hive to use. (Defaults to HKEY_LOCAL_MACHINE)</param>
		/// <param name="view">A RegistryView value specifying which Registry view to reference.</param>
		/// <returns>A count of the number of keys/values found in the requested registry location. Or -1 if the request failed for any reason.</returns>
		/// <remarks>Any errors encountered in the process are stored in RegistryKey.LastError.</remarks>
		public static int ValueCount(string subKey = "", RegistryHive? hive = null, RegistryView? view = null) =>
		...
		/// <param name="subKey">A string specifying the SubKey to dereference. (Defaults to the value in RegistryAccess.DEFAULT_SUBKEY)</param>
		/// <param name="hive">A RegistryKey value specifying which hive to use. (Defaults to the value in RegistryAccess.DEFAULT_REGISTRY_KEY)</param>
		/// <returns>TRUE if the specified registry entry contains subkeys, otherwise FALSE.</returns>
		/// <remarks>Any errors encountered in the process are stored in RegistryKey.LastError.</remarks>
		...
		/// <param name="hive">A RegistryHive value specifying which Hive to search in (default is HKEY_LOCAL_MACHINE).</param>
		/// <param name="createIfNot">A bool value that, if set to TRUE, will cause the specified key to be created if it doesn't exist (default to FALSE).</param>
		/// <param name="view">A RegistryView value specifying which View to use.</param>
		...
		public static bool KeyExists(string subKey = "", RegistryHive? hive = null, bool createIfNot = false, RegistryView? view = null)
		{
			...
			try
			{
				...
				LastError = (reg is null) ? "Could not find the requested key (\"" + RegistryHiveToAbbr(hive, true) + subKey + "\") in the specified view (" + view.ToString() + ")." : "";
				return !(reg is null);
			}
			catch(Exception e) { LastError = e.Message; }
			return false;
		...
		/// <param name="hive">A RegistryHive value specifying which Hive to search in (default is HKEY_LOCAL_MACHINE).</param>
		/// <param name="view">A RegistryView value specifying which View to use.</param>
		/// <returns>TRUE if a key was found in the specified location, otherwise FALSE.</returns>
		public static bool ValueExists(string valueName, string subKey = "", RegistryHive? hive = null, RegistryView? view = null)
		{
			LastError = ""; // Clear the LastError value.
			RegistryKey key = FetchKey(subKey, hive, view);
			if (key is null)
				LastError = "Could not find the requested key (\"" + RegistryHiveToAbbr(hive, true) + subKey + "\") in the specified view (" + view.ToString() + ").";
			else
				if (key.GetValue(valueName) is null)
				LastError = "Could not find the requested value (\"" + valueName + "\") in the specified key / hive / view (\"" + RegistryHiveToAbbr(hive, true) + subKey + "\", " + view.ToString() + ").";
			else
				return true;
			return false;
		}
		...
		/// <param name="hive">A RegistryHive value specifying which Hive to search in (default is HKEY_LOCAL_MACHINE).</param>
		/// <param name="view">A RegistryView value specifying which View to use.</param>
		/// <returns>TRUE if the specified subKey has a default value.</returns>
		public static bool HasDefaultValue(string subKey = "", RegistryHive? hive = null, RegistryView? view = null)
		{
			...
			{
				RegistryKey key = FetchKey(subKey, hive, view);
				return !((key is null) || (key.GetValue("") is null));
			}
			catch (Exception e) { LastError = e.Message; }
			return false;
		}
		...
		/// <param name="hive">A RegistryHive value specifying which Hive to search in (default is HKEY_LOCAL_MACHINE).</param>
		/// <param name="view">A RegistryView value specifying which View to use.</param>
		/// <returns>The default value of the subkey, if one exists, otherwise NULL.</returns>
		...
		/// <summary>
		/// Attempts to creates a specified registry key and returns a RegistryKey value from it.
		...
		/// <param name="hive">A RegistryHive value specifying which Hive to search in (default is HKEY_LOCAL_MACHINE).</param>
		/// <param name="view">A RegistryView value specifying which View to use.</param>
		/// <returns>A RegistryKey object if the specified Key was created, or already existed, otherwise NULL.</returns>
		public static RegistryKey CreateKey(string subKey = "", RegistryHive? hive = null, RegistryView? view = null)
		{
			// Validate the incoming subKey string for the default value
			subKey = string.IsNullOrEmpty(subKey.Trim()) ? DEFAULT_SUBKEY : subKey.Trim();
		...
		/// <summary>Retrieves a specified RegistryKey object.</summary>
		/// <param name="subKey">A string specifying the Key to retrieve.</param>
		/// <param name="hive">A RegistryHive value specifying which Hive to search in (default is HKEY_LOCAL_MACHINE).</param>
		/// <param name="view">A RegistryView value specifying which View to use.</param>
		/// <returns>A populated RegistryKey object if the specified Key was found, otherwise NULL.</returns>
		...
		/// <summary>Creates a RegistryValue within a specified Registry Key (which must exist!)</summary>
		...
		/// <param name="type">A RegistryValueKind designation for the value (default = string).</param>
		...
		/// <param name="hive">The RegistryHive to reference.</param>
		/// <param name="view">Which Registry to use.</param>
		/// <returns>If the value is found, it's RegistryValueKind, otherwise RegistryValueKind.None.</returns>
		/// <remarks>
		...
		public static RegistryHive ParseHiveAbbr(string value) =>
			Regex.Replace( value, @"[^acdefghiklmnoprstuy]", "", RegexOptions.IgnoreCase ).ToLower() switch
			{
				...
				//"hkeydynamicdata"or"dynamicdata"or"dynamic"or"hkdd" => RegistryHive.DynData,
		...
		//	switch ( Regex.Replace(value, @"[^acdefghiklmnoprstuy]", "", RegexOptions.IgnoreCase).ToLower())
		//	{
		...
		//	}
		//	throw new InvalidOperationException("The value provided (\"" + value + "\") does not conform to an accepted RegistryHive abbreviation.");
		//}

		/// <summary>Converts a RegistryHive enum into an appropriate abbreviated string value.</summary>
		/// <param name="value">The RegistryHive value to parse.</param>
		/// <param name="dressItUp">
		/// If set to TRUE (default = FALSE) the returned value will be enclosed in brace brackets and be followed by a colon
		/// (i.e. "RegistryHive.LocalMachine" => "{HKLM}:").
		/// </param>
		/// <returns>A string containing an appropriate designation for the supplied RegistryHive value.</returns>
		public static string RegistryHiveToAbbr(RegistryHive? value, bool dressItUp = false)
		{
