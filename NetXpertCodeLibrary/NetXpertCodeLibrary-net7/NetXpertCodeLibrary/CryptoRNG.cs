...
namespace NetXpertCodeLibrary
{
	public static class CryptoRNG
	{
		...
		/// <returns>TRUE if <typeparamref name="T"/> is one of the C# Integer classes.</returns>
		...
		private static T MinValue<T>() where T : IBinaryInteger<T>
		{
		...
		private static T MaxValue<T>() where T : IBinaryInteger<T>
		{
			if ( IsIntegerType<T>() )
		...
		/// <summary>Creates a percentage (type <seealso cref="decimal"/>) from any provided <seealso cref="IntegerTypes"/> type.</summary>
		...
		private static double Multiplier<T>( T value ) where T : IBinaryInteger<T>
		{
			if ( IsIntegerType<T>() )
			{
		...
		/// <exception cref="InvalidCastException">Thrown if the Generic type isn't one of C#'s Integer types.</exception>
		private static T Generate<T>() where T : IBinaryInteger<T>
		{
			if ( IsIntegerType<T>() )
		...
		/// <exception cref="InvalidCastException">Thrown if the Generic type isn't one of C#'s Integer types.</exception>
		public static T[] Generate<T>( int count ) where T : IBinaryInteger<T>
		{
			if ( count > 0 )
			{
		...
		/// <summary>Creates an array of cryptographically-safe random numbers with the specified length within the specified range.</summary>
		/// <typeparam name="T">Specifies the Integer type of the numbers to be generated.</typeparam>
		/// <param name="count">The number of elements to populate the array with.</param>
		/// <returns>An array of specified values within the provided range, of the specified type, and length.</returns>
		/// <exception cref="InvalidCastException">Thrown if the Generic type isn't one of C#'s Integer types.</exception>
		public static T[] Generate<T>( int count, T lowerValue, T upperValue ) where T : IBinaryInteger<T>
		{
			if ( count > 0 )
			{
				T[] result = new T[ count ];
				for ( int i = 0; i < count; i++ )
					result[ i ] = Generate<T>(lowerValue,upperValue);
		...
		public static T Generate<T>( T lowerValue, T upperValue ) where T : IBinaryInteger<T> =>
		...
		private static T Rangify<T>(T value, T lowerValue, T upperValue ) where T : IBinaryInteger<T>
		{
			if (IsIntegerType<T>())
			{
