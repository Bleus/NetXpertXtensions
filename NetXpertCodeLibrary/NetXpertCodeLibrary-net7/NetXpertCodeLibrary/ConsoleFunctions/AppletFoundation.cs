...
namespace NetXpertCodeLibrary.ConsoleFunctions
{
	public static class Applets
	{
		...
		/// <returns>The highest non-abstract ancestor class of the provided Type or NULL if none exist.</returns>
		...
		public static OperationalStates ExceptionDump( Exception e, bool debugMode = false )
		{
			...
			{
				bool innerExc = ie.InnerException is not null;
				...
				if ( debugMode) 
				{
					...
					TextElementRule rule1 = new( /* parameters */ /* language=regex */
						@"([(,]?)([\w&]+)([ ]+)", "$1{D}$2{7}$3" );
					TextElementRule rule2 = new( /* fnName */ /* language=regex */
	...
	public class ReflectedApplet
	{
		...
		public override string ToString() =>
			$"{((this._object == null) ? "NULL" : this._object.GetType())};";
			...
			get
			{
				...
				if (this._rawXml.Length > 0)
				{
					try { result.LoadXml(this._rawXml); }
						...
						result.LoadXml( $"{XML.HEADER}<xmlConversionError>{e.Message}</xmlConversionError");
					}
				}
				return result;
			}
		...
		public Applets.OperationalStates Execute(Command cmd, Ranks asRank)
		{
			...
			if (this.HasSwitch("ver"))
			{
				if ( descriptor.Description.Length > 0 )
					Tec( "{7,7}&quot;{$1}$3{7,rn}&quot;", new object[] { new CliColor( "9" ), MiscellaneousExtensions.ExecutableName, Descriptor().Description } );
