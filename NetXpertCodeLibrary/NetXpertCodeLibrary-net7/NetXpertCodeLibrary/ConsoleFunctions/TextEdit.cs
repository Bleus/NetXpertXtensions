...
namespace NetXpertCodeLibrary.ConsoleFunctions
{
		...
		public int DelCharLeft(int position)
		{
			...
			if (_value.Length > 0)
			{
				...
				if ( position == _value.Length )
				{
					...
					if ( position.InRange(_value.Length, 0, NetXpertExtensions.Classes.Range.BoundaryRule.Exclusive) )
					{
						if (">\r\n".IndexOf( _value[ position ] ) > 0)
						{
							while (">\r\n".IndexOf( _value[ position ] ) > 0)
	...
	public class TextEditField
	{
		...
		{
			position = position.InRange( _value.Length, 0, NetXpertExtensions.Classes.Range.BoundaryRule.Inclusive ) ? position : (position < 0) ? InsertPosition : _value.Length;
		...
		{
			bool result = false;
			if (!ReadOnly && InsertPosition.InRange( _value.Length, 0, NetXpertExtensions.Classes.Range.BoundaryRule.Loop ))
			{
